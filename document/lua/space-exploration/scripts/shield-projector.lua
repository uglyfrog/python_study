...
function ShieldProjector.rebuild_cache()
  global.shield_projector_buffer_capacity = game.entity_prototypes[ShieldProjector.name_entity].electric_energy_source_prototype.buffer_capacity
  global.shield_projector_barrier_max_health = game.entity_prototypes[ShieldProjector.name_barrier].max_health
end
Event.addListener("on_init", ShieldProjector.rebuild_cache, true)
Event.addListener("on_configuration_changed", ShieldProjector.rebuild_cache, true)
...
function ShieldProjector.on_entity_created(event)
  ...
  if entity.name == ShieldProjector.name_entity then
    ...
    local max_energy = global.shield_projector_buffer_capacity
...
function ShieldProjector.update(shield_projector)
  ...
  local energy_p = energy / global.shield_projector_buffer_capacity
  ...
  if shield_projector.is_projecting then
    ...
    if energy_p > ShieldProjector.threshold_project then
      ...
      for _, orientation_offset in pairs{-1/24, 0, 1/24} do
        ...
        global.shield_projector_barriers = global.shield_projector_barriers or {}
        global.shield_projector_barriers[barrier.unit_number] = shield_projector.unit_number
      end
    end
  end
end
...
function ShieldProjector.repair_barrier(shield_projector, barrier, free_regen)
  ...
  local max_health = global.shield_projector_barrier_max_health
...
function ShieldProjector.on_entity_damaged (event)
  if event.entity and event.entity.valid and event.entity.name == ShieldProjector.name_barrier then
    if not (global.shield_projector_barriers and global.shield_projector_barriers[event.entity.unit_number]) then
      ShieldProjector.rebuild_barrier_index()
    end
    local shield_projector = global.shield_projectors
            and global.shield_projectors[global.shield_projector_barriers[event.entity.unit_number]]
...
function ShieldProjector.rebuild_barrier_index()
  global.shield_projector_barriers = {}
  for _, shield_projector in pairs(global.shield_projectors) do
    for _, barrier in pairs(shield_projector.barriers) do
