...
function Weapon.on_trigger_created_entity(event)
  ...
  elseif event.entity.name == mod_prefix.."cryogun-trigger" then
    ...
    if safe_pos then
      ...
      if spaceship then
        Log.trace("test")
        Spaceship.add_as_particle(spaceship, ice, Spaceship.ice_particle)
      end
    end
...
function Weapon.plague_tick(tick_task)
  ...
  if game.tick > tick_task.started + settings.global["se-plague-max-runtime"].value * 60 * 60 then -- 15 minutes
    tick_task.valid = false
