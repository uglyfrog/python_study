...
function RemoteView.get_stack_limit(stack) -- must be lua item stack, not simple stack
  ...
  if name == "artillery-targeting-remote"
    --or name == "ion-cannon-targeter" -- still bugged
    or name == "spidertron-remote" or name == "spidertron-remote-waypoint" or name == "spidertron-remote-patrol" then -- type == "capsule"
      return stack.count
  end
  if name == "red-wire"
    or name == "green-wire"
...
function RemoteView.gui_open (player)
  ...
  local starmap = right_flow.add{type="sprite-button", name=RemoteView.name_button_starmap, sprite = "se-map-gui-starmap", style="informatron_close_button", tooltip={"space-exploration.interstellar-map"}}
...
function RemoteView.on_gui_click (event)
  ...
  if root then -- remote view
    ...
    elseif element.name == RemoteView.name_button_starmap then
