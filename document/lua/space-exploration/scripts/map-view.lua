...
InterstellarMap.asteroid_scale = 0.25 * InterstellarMap.scale
...
function InterstellarMap.start(player, from_zone)
...
function InterstellarMap.render_asteroid_field(player, surface, zone, playerdata, spaceships)
  ...
  local object_id = rendering.draw_sprite{
    ...
    surface = surface,
    target = map_object.entity,
    x_scale = 4 * InterstellarMap.asteroid_scale,
...
function SystemMap.start(player, star, from_zone)
      ...
      if map_object then
        if map_object.command == "system" then
          ...
          if Zone.is_visible_to_force(map_object.zone, player.force.name ) then
            ...
            player.print("Insufficient data, launch more satellites to view the system.")
          end
...
function MapView.gui_open(player, zone)
  ...
  if zone.type ~= "star" and zone.orbit then
    local land = right_flow.add{type="sprite-button", name=MapView.name_gui_zone_details_root.."_land", sprite = Zone.get_icon(zone), style="button"}
    land.style.width = 40
    land.style.height = 40
    land.style.bottom_margin = -5
    land.enabled = false
    local orbit = right_flow.add{type="sprite-button", name=MapView.name_gui_zone_details_root.."_orbit", sprite = Zone.get_icon(zone.orbit), style="button"}
    orbit.style.width = 40
    orbit.style.height = 40
    orbit.style.right_margin = 6
    orbit.style.bottom_margin = -5
  end
  local close = right_flow.add{type="sprite-button", name=MapView.name_gui_zone_details_root.."_close", sprite = "utility/close_white", style="informatron_close_button"}
...
function MapView.on_gui_click(event)
  ...
  if event.element.name == MapView.name_button_overhead_interstellar then
  ...
  if button_flow then
    if settings.get_player_settings(player)[MapView.name_setting_overhead_interstellar].value == true then
      if not button_flow[MapView.name_button_overhead_interstellar] then
        button_flow.add{type="sprite-button", name=MapView.name_button_overhead_interstellar, sprite="se-map-gui-starmap", tooltip = {"space-exploration.interstellar-map"}}
      end
    else
