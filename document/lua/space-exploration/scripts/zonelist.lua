...
function Zonelist.gui_create_zone_content(player, parent)
  ...
  zone_content_pane.style.extra_top_padding_when_activated = 0
  zone_content_pane.style.extra_bottom_padding_when_activated = 0
  zone_content_pane.style.extra_left_padding_when_activated = 0
  zone_content_pane.style.extra_right_padding_when_activated = 0
  zone_content_pane.style.extra_top_margin_when_activated = 0
  zone_content_pane.style.extra_bottom_margin_when_activated = 0
  zone_content_pane.style.extra_left_margin_when_activated = 0
  zone_content_pane.style.extra_right_margin_when_activated = 0
...
function Zonelist.gui_update_selected(player_index) -- only call when something changed
  ...
  Zonelist.gui_update_zone_content(player, window, selected_zone)
end

function Zonelist.gui_update_zone_content(player, window, zone)
  local force_name = player.force.name
  local forcedata = global.forces[force_name]
  local playerdata = get_make_playerdata(player)
  local zone_force_assets = Zone.get_force_assets(force_name, zone.index)
  ...
  if zone then
    ...
    if zone_content_pane then
      ...
      if zone.type ~= "spaceship" then
        ...
        if has_glyph or has_visisted or has_launchpad or has_landingpad or has_ruin then
        ...
        if not zone.surface_index then
          ...
          trim_button.tooltip = {"space-exploration.trim-zone-button-no-surface-tooltip"}
        end
...
function Zonelist.gui_update_list(player_index) -- only call when something changed
  ...
  local function format_cell(element, is_heading)
    ...
    if element.name == "cell_flags" then
  ...
  if zonelist_scroll then
    for i, zone in pairs(zones_list) do
      ...
      if zone.is_homeworld then
        cell_resource.caption = "[img=item/se-core-fragment-omni]"
        cell_resource.tooltip = {"item-name.se-core-fragment-omni"}
      end
      ...
      if zone.interburbulator or zone.ruins then
        cell_flags.caption = cell_flags.caption .. "[img=virtual-signal/se-ruin]"
      end
  ...
  if button_flow then
    if settings.get_player_settings(player)[Zonelist.name_setting_overhead_explorer].value == true then
      if not button_flow[Zonelist.name_button_overhead_explorer] then
        button_flow.add{type="sprite-button", name=Zonelist.name_button_overhead_explorer, sprite="se-map-gui-universe-explorer", tooltip = {"space-exploration.zonelist-window-title"}}
      end
    else
