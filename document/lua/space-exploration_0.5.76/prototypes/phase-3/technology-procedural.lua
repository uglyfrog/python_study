...
local function update_tech_for_complexity(tech, complexity, space_flavour)
  ...
  if complexity >= 10 then
    data_util.tech_add_ingredients_with_prerequisites(tech.name, { data_util.mod_prefix .. "deep-space-science-pack-1" })
    multiplier = 0.2
  end
  if complexity >= 11 then
    data_util.tech_add_ingredients_with_prerequisites(tech.name, { data_util.mod_prefix .. "deep-space-science-pack-2" })
    multiplier = 0.15
  end
  if complexity >= 12 then
    data_util.tech_add_ingredients_with_prerequisites(tech.name, { data_util.mod_prefix .. "deep-space-science-pack-3" })
    multiplier = 0.12
  end
  if complexity >= 13 then
    data_util.tech_add_ingredients_with_prerequisites(tech.name, { data_util.mod_prefix .. "deep-space-science-pack-4" })
    multiplier = 0.1
  end
  if complexity >= 14 then
